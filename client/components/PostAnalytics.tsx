...
const PostAnalytics : FC<props> = (props) => {
  ...
  const getViewsData = async () => {
    let response;
    try {
        const endpoint = `${api}/coursePosts/views/${id}`;
        response = await axios.get(endpoint);
    } catch {
        try {
            const endpoint = `${api}/activityPosts/views/${id}`;
            response = await axios.get(endpoint);
        } catch {
  ...
  const getDemographicsData = async () => {
    let response;
    try {
        const endpoint = `${api}/coursePosts/demographics/${id}`;
        const params = { params: { start: getDaysAgo(timeScaleToDays()) }};
        response = await axios.get(endpoint, params);
    } catch (error) {
        try {
            const endpoint = `${api}/activityPosts/demographics/${id}`;
            const params = { params: { start: getDaysAgo(timeScaleToDays()) }};
            response = await axios.get(endpoint, params);
        } catch (error) {
            console.error("Cannot find post")
